...
import type { DamageItem, DamageType, FieldConfig, GlobalConfig, LackberechnungsartType, MinderwertRow, PaintMeasurement, PhotoComponentAssignment, RepairMethodType, RepairPosition, RepairTableEntry, RepairType, ReportData, ReportPhoto, SignatureNames, Signatures, TireInfo, VehicleCategoryType } from '../types/report.types';
...
interface ReportStore extends ReportData {
    ...
    assignPhotoToComponent: (photoIdOrData: string, targetComponentId: string | null) => void;
    assignPhotoToComponents: (photoIdOrData: string, targetComponentIds: string[]) => void;
    togglePhotoComponentAssignment: (photoIdOrData: string, targetComponentId: string) => void;
    unassignPhotoFromComponent: (photoIdOrData: string, targetComponentId: string) => void;
    getPhotoAssignment: (photoDataOrId: string) => PhotoComponentAssignment | null;
    getPhotoAssignments: (photoDataOrId: string) => PhotoComponentAssignment[];
...
const reportStoreCreator: StateCreator<ReportStore> = (set, get) => ({
    ...
    assignPhotoToComponent: (photoIdOrData, targetComponentId) => set(state => {
        let photo = state.photos.find(p => p.id === photoIdOrData || p.data === photoIdOrData);
        const photoData = photo ? photo.data : photoIdOrData;
        ...
        if (!photo) {
            photo = {
                ...
                damageId: targetComponentId || undefined,
                damageIds: targetComponentId ? [targetComponentId] : [],
                includeInPdf: true
            };
        ...
        if (targetComponentId === null) {
            // Complete unassign from all components
            return {
                photos: updatedPhotos,
                minderwertRows: state.minderwertRows.map(r => ({
                    ...r,
                    images: (r.images || []).filter(img => img !== photoData)
                })),
                damages: state.damages.map(d => ({
                    ...d,
                    images: (d.images || []).filter(img => img !== photoData)
                }))
            };
        }

        // Add to the specific component without stripping others
        let updatedMinderwertRows = state.minderwertRows.map(r => {
            if (r.id === targetComponentId || r.bodyPart === targetComponentId) {
                const cur = r.images || [];
                return cur.includes(photoData) ? r : { ...r, images: [...cur, photoData] };
            }
            return r;
        });

        let updatedDamages = state.damages.map(d => {
            if (d.id === targetComponentId) {
                const cur = d.images || [];
                return cur.includes(photoData) ? d : { ...d, images: [...cur, photoData] };
            }
            return d;
        });
    ...
    assignPhotoToComponents: (photoIdOrData, targetComponentIds) => set(state => {
        let photo = state.photos.find(p => p.id === photoIdOrData || p.data === photoIdOrData);
        const photoData = photo ? photo.data : photoIdOrData;
        ...
        if (!photo) {
            photo = {
                ...
                label: 'Foto',
                fileName: `photo_${Date.now()}.jpg`,
                damageId: targetComponentIds[0] || undefined,
                damageIds: targetComponentIds,
                includeInPdf: true
            };
            updatedPhotos.push(photo);
        } else {
        ...
        const updatedMinderwertRows = state.minderwertRows.map(r => {
            const shouldHave = targetSet.has(r.id) || (r.bodyPart && targetSet.has(r.bodyPart));
            const cur = r.images || [];
            const hasIt = cur.includes(photoData);

            if (shouldHave && !hasIt) {
                return { ...r, images: [...cur, photoData] };
            }
            if (!shouldHave && hasIt) {
                return { ...r, images: cur.filter(img => img !== photoData) };
            }
            return r;
        });

        const updatedDamages = state.damages.map(d => {
            const shouldHave = targetSet.has(d.id);
            const cur = d.images || [];
            const hasIt = cur.includes(photoData);

            if (shouldHave && !hasIt) {
                return { ...d, images: [...cur, photoData] };
            }
            if (!shouldHave && hasIt) {
                return { ...d, images: cur.filter(img => img !== photoData) };
            }
            return d;
        });
    ...
    togglePhotoComponentAssignment: (photoIdOrData, targetComponentId) => set(state => {
        let photo = state.photos.find(p => p.id === photoIdOrData || p.data === photoIdOrData);
        const photoData = photo ? photo.data : photoIdOrData;

        // Check whether target component currently has this photoData
        const mwRow = state.minderwertRows.find(r => r.id === targetComponentId || r.bodyPart === targetComponentId);
        const damage = state.damages.find(d => d.id === targetComponentId || d.bodyPart === targetComponentId);
        const isCurrentlyAssigned = (mwRow?.images || []).includes(photoData) || (damage?.images || []).includes(photoData);

        let updatedMinderwertRows = state.minderwertRows.map(r => {
            if (r.id === targetComponentId || r.bodyPart === targetComponentId) {
                const cur = r.images || [];
                return isCurrentlyAssigned
                    ? { ...r, images: cur.filter(img => img !== photoData) }
                    : (cur.includes(photoData) ? r : { ...r, images: [...cur, photoData] });
            }
            return r;
        });

        let updatedDamages = state.damages.map(d => {
            if (d.id === targetComponentId || d.bodyPart === targetComponentId) {
                const cur = d.images || [];
                return isCurrentlyAssigned
                    ? { ...d, images: cur.filter(img => img !== photoData) }
                    : (cur.includes(photoData) ? d : { ...d, images: [...cur, photoData] });
            }
            return d;
        });

        // If assigning and neither row nor damage exists yet:
        if (!isCurrentlyAssigned && !mwRow && !damage) {
            if (BODY_PARTS.some(b => b.id === targetComponentId)) {
                updatedMinderwertRows.push({
                    id: targetComponentId,
                    bodyPart: targetComponentId,
                    damage: '' as any,
                    repairMethod: '' as any,
                    repairCost: 0,
                    repairCostBrutto: 0,
                ...
                updatedDamages.push({
                    ...
                    sparePartsBrutto: 0,
                    images: [photoData],
                    reparaturweg: 'Karosserie',
                    damageType: 'Schaden',
                    damageSeverity: 'leicht',
                    isUserAdded: true,
                    damageClassification: 'vorhanden'
                } as any);
            }
        }

        // Compute new set of all assigned components for this photo
        const allAssignedIds: string[] = [];
        updatedMinderwertRows.forEach(r => {
            if ((r.images || []).includes(photoData)) {
                allAssignedIds.push(r.bodyPart || r.id);
            }
        });
        updatedDamages.forEach(d => {
            if ((d.images || []).includes(photoData)) {
                allAssignedIds.push(d.bodyPart || d.id);
            }
        });
        ...
        if (!photo) {
            photo = {
                ...
                label: 'Foto',
                fileName: `photo_${Date.now()}.jpg`,
                damageId: allAssignedIds[0] || undefined,
                damageIds: allAssignedIds,
                includeInPdf: true
            };
            updatedPhotos.push(photo);
        } else {
    ...
    unassignPhotoFromComponent: (photoIdOrData, targetComponentId) => set(state => {
        let photo = state.photos.find(p => p.id === photoIdOrData || p.data === photoIdOrData);
        const photoData = photo ? photo.data : photoIdOrData;
        ...
        let updatedPhotos = state.photos.map(p => {
            if (p.id === photo?.id || p.data === photoData) {
                return {
                    ...p,
                    damageId: allAssignedIds[0] || undefined,
                    damageIds: allAssignedIds
    ...
    getPhotoAssignment: (photoDataOrId) => {
        const assignments = get().getPhotoAssignments(photoDataOrId);
        return assignments.length > 0 ? assignments[0] : null;
    },

    getPhotoAssignments: (photoDataOrId) => {
        const state = get();
        const photo = state.photos.find(p => p.id === photoDataOrId || p.data === photoDataOrId);
        const photoData = photo ? photo.data : photoDataOrId;

        const results: PhotoComponentAssignment[] = [];
        const seenIds = new Set<string>();

        // 1. Scan minderwertRows for this image
        state.minderwertRows.forEach(r => {
            if ((r.images || []).includes(photoData)) {
                const key = r.bodyPart || r.id;
                if (!seenIds.has(key)) {
                    seenIds.add(key);
                    results.push({
                        id: r.id,
            ...
            if ((d.images || []).includes(photoData)) {
                if (!seenIds.has(d.id)) {
                    ...
                    results.push({
                        ...
                        type: 'damage',
                        bodyPart: d.bodyPart,
                        label: d.bodyPart || d.description || d.id,
        ...
        // 3. Fallback to photo.damageIds or photo.damageId if not found in arrays
        if (photo?.damageIds && photo.damageIds.length > 0) {
            photo.damageIds.forEach(targetId => {
                if (!seenIds.has(targetId)) {
                    const mwRow = state.minderwertRows.find(r => r.id === targetId || r.bodyPart === targetId);
                    if (mwRow) {
                        seenIds.add(targetId);
                        results.push({
                            id: mwRow.id,
                            type: 'minderwert',
                            bodyPart: mwRow.bodyPart,
                            label: mwRow.bodyPart,
                            description: mwRow.damage
                        });
                    } else {
                        const damage = state.damages.find(d => d.id === targetId);
                        if (damage) {
                            seenIds.add(targetId);
                            results.push({
                                id: damage.id,
                                ...
                                label: damage.bodyPart || damage.description || damage.id,
        ...
        } else if (photo?.damageId && !seenIds.has(photo.damageId)) {
            const targetId = photo.damageId;
            const mwRow = state.minderwertRows.find(r => r.id === targetId || r.bodyPart === targetId);
            if (mwRow) {
                seenIds.add(targetId);
                results.push({
    ...
    removeMinderwertPhoto: (id: string, photoIndex: number) => set(state => {
        const row = state.minderwertRows.find(r => r.id === id || r.bodyPart === id);
        const removedData = row?.images?.[photoIndex];
        return {
            minderwertRows: state.minderwertRows.map(r => {
                if (r.id !== id && r.bodyPart !== id) return r;
                const newImages = [...(r.images || [])];
                newImages.splice(photoIndex, 1);
                return { ...r, images: newImages };
            }),
            photos: removedData
                ? state.photos.map(p => (p.data === removedData && (p.damageId === id || p.damageId === row?.bodyPart)) ? { ...p, damageId: undefined } : p)
                : state.photos
        };
    }),
        ...
        reportStoreCreator,
        {
            ...
            partialize: (state) => {
                const {
                    ...
                    movePhoto,
                    reorderPhoto,
                    assignPhotoToComponent,
                    assignPhotoToComponents,
                    togglePhotoComponentAssignment,
                    unassignPhotoFromComponent,
                    getPhotoAssignment,
                    getPhotoAssignments,
                    setBodyPartDamage,
                    addPaintMeasurement,
