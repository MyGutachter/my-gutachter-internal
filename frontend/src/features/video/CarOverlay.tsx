...
export const getPartIdFromKey = (key: string): string => {
...
export const CarOverlay: React.FC<CarOverlayProps> = (props) => {
    ...
    const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        setIsDragging(false);
        // Allow the following click event to run if this was not a real drag
    ...
    const actualSelectedParts = useMemo(() => {
        return selectedParts.map(getPartIdFromKey);
    }, [selectedParts]);

    const damagesByLocation = useMemo(() => {
        const grouped: Record<string, DamageItem[]> = {};
        selectedParts.forEach((partKey, index) => {
    ...
    const getPartColor = (partId: string): string => {
        if (isAdd) {
            if (actualSelectedParts.includes(partId)) {
    ...
    const getPartOpacity = (partId: string) => {
        return actualSelectedParts.includes(partId) ? 0.9 : 0.7;
    };
    ...
    const renderCarView = () => (
        ...
        >
            ...
            <path
                className="st0 part-path"
                style={{ fill: getPartColor('Right-hand_exterior_mirror'), opacity: getPartOpacity('Right-hand_exterior_mirror') }}
                onClick={() => handlePartClick('Right-hand_exterior_mirror', 'Right-hand exterior mirror')}
                onMouseEnter={() => handlePartMouseEnter('Right-hand_exterior_mirror', 'Right-hand exterior mirror')}
                onMouseLeave={handlePartMouseLeave}
                                ...
                                <div key={partKey} className="flex items-center gap-2 bg-red-500/10 text-red-500 px-3 py-1 rounded-full">
                                    ...
                                    {savedScreenshots[partKey] && (
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onViewScreenshot(savedScreenshots[partKey]);
                                            }}
